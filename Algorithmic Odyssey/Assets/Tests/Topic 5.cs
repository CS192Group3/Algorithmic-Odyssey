...
namespace Topic5
{
    public class TestSuite : InputTestFixture
    {
        ...
        {
            GameObject topicButton = GameObject.Find("Canvas/Background/Main Panel - Shadow/Wood/Button Container/Topic 5");
            string sceneName = SceneManager.GetActiveScene().name;
            Assert.That(sceneName, Is.EqualTo("ReviewScene"));

            ClickAction(topicButton);
            yield return new WaitUntil(() => SceneManager.GetActiveScene().name == "Topic5");
            yield return new WaitForSeconds(2f);

            sceneName = SceneManager.GetActiveScene().name;
            Assert.That(sceneName, Is.EqualTo("Topic5"));
